...
def tambah_siswa(nama, asal_sekolah):
    global kelas_counter # melacak kelas yang ada
...
def tampilkan_semua_kelas():
    if len(kelas) == 0:
        ...
        for i, k in enumerate(kelas, start=1): #Fungsi enumerate(kelas, start=1) menghasilkan indeks yang dimulai dari 1
            print(f"\nKelas {i}:") #Setiap iterasi menampilkan nama kelas dengan format Kelas i
            for siswa in k:
                print(f" - {siswa['nama']} ({siswa['asal_sekolah']})") #siswa diasusmsikan sebuah dictionary yang memiliki 2 kunci
